def prompt_template(data: str, num_records: int = 5):
    ...
    **Explanation:**
    ...
    - Do **NOT** wrap each item in its own code block — there must be **only one code block** and **one JSON array**.
    - Do **NOT** repeat ` ```json ` multiple times — only **one set of backticks** enclosing the entire JSON array.
    - Do **NOT** add any explanations, extra text, or notes before or after the JSON. **Only return the JSON.**
    - You MUST generate exactly {num_records} Q&A objects.
